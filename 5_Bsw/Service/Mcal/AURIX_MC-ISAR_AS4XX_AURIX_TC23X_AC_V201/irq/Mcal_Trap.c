...
EcuHwCtrlM_S_EcuHwErrData_t Ecu_Hw_Pflash_EccError = {
	ERR_INHIBIT, /* flag */
	0, /* count */
	PFLASH_ECCERROR_DET_COUNT, /* limit */
};

EcuHwCtrlM_S_EcuHwErrData_t Ecu_Hw_Dflash_EccError = {
	ERR_INHIBIT, /* flag */
	0, /* count */
	DFLASH_ECCERROR_DET_COUNT, /* limit */
};

EcuHwCtrlM_S_EcuHwErrData_t Ecu_Hw_Ram_DSPR_EccError = {
	ERR_INHIBIT, /* flag */
	0, /* count */
	RAM_ECCERROR_DET_COUNT, /* limit */
};

EcuHwCtrlM_S_EcuHwErrData_t Ecu_Hw_Ram_PSPR_EccError = {
	ERR_INHIBIT, /* flag */
	0, /* count */
	RAM_ECCERROR_DET_COUNT, /* limit */
};

void EcuHwCtrlM_Sec_CommonCheck(EcuHwCtrlM_S_EcuHwErrData_t * ErrData, uint8 Check);
...
void _trap_4( void )
{
  ...
  if (entry_cnt == 0)
  {
	  Ecu_Hw_Ram_PSPR_EccError.flag = ERR_NONE;
	  Ecu_Hw_Ram_DSPR_EccError.flag = ERR_NONE;
	  Ecu_Hw_Pflash_EccError.flag = ERR_NONE;
	  Ecu_Hw_Dflash_EccError.flag = ERR_NONE;
	  entry_cnt = 1;
  }
...
/***********Trap Handler mechanism****************/
    volatile static uint8 trap_cnt = 0;
    uint32 datr_status = 0;
    uint32 dstr_status = 0;
    uint32 pstr_status = 0;
    uint32 pietr_status = 0;
    uint32 dietr_status = 0;
    uint32 error_addr = 0;

  	trap_cnt++;
    pstr_status = __mfcr(CPU_PSTR) ;
    pietr_status = __mfcr(CPU_PIETR);
    datr_status = __mfcr(CPU_DATR) ;
    dstr_status = __mfcr(CPU_DSTR) ;
    dietr_status = __mfcr(CPU_DIETR);

    if ((pstr_status != 0) || (pietr_status != 0) || (datr_status != 0) || (dstr_status != 0) || (dietr_status != 0))
      {
        if (pstr_status != 0)
        {
          //error_addr = (uint32)__getA11();
        }
        else if (pietr_status != 0)
        {
          error_addr = __mfcr(CPU_PIEAR);
        }
        else if ((datr_status != 0) || (dstr_status != 0))
        {
          error_addr = __mfcr(CPU_DEADD);

         // __mtcr (CPU_DATR, 0x0); /*Need to clear for DAE Trap*/
        }
        else //(dietr_status != 0)
        {
          error_addr = __mfcr(CPU_DIEAR);
        }

        if (((error_addr >= 0x80000000) && (error_addr <= 0x801fffff)) /*|| ((error_addr >= 0xa0000000) && (error_addr <= 0xa03fffff))*/)
        {
            //P-Flash Error
            EcuHwCtrlM_Sec_CommonCheck(&Ecu_Hw_Pflash_EccError, TRUE);
        }
        else if (((error_addr >= 0xaf000000) && (error_addr <= 0xaf103fff)) || ((error_addr >= 0xaf110000) && (error_addr <= 0xaf11ffff)))
        {
            //D-Flash Error
            EcuHwCtrlM_Sec_CommonCheck(&Ecu_Hw_Dflash_EccError, TRUE);
        }
        else if (/*((error_addr >= 0x50000000) && (error_addr <= 0x5001dfff)) || ((error_addr >= 0x60100000) && (error_addr <= 0x60107fff)) ||*/
                        ((error_addr >= 0x70100000) && (error_addr <= 0x70101FFF)))
        {
            //PSPR(Program Scratch Pad RAM) Error
            EcuHwCtrlM_Sec_CommonCheck(&Ecu_Hw_Ram_PSPR_EccError, TRUE);
        }
        else if (/*((error_addr >= 0x50100000) && (error_addr <= 0x50107fff)) || ((error_addr >= 0x60000000) && (error_addr <= 0x6001dfff)) ||*/
                        ((error_addr >= 0x70000000) && (error_addr <= 0x7002DFFF)))
        {
            //DSPR(Data Scract Pad RAM) Error
            EcuHwCtrlM_Sec_CommonCheck(&Ecu_Hw_Ram_DSPR_EccError, TRUE);
        }
        else
        {
            ;
...
void EcuHwCtrlM_Sec_CommonCheck(EcuHwCtrlM_S_EcuHwErrData_t * ErrData, uint8 Check)
{
    if(ErrData->flag!=ERR_INHIBIT)
    {
        /* Convert Condition Data Type From Uint8 To Boolean */
        if(!!Check)
        {
            /* Increase Check Count When Condition is True. */
            ErrData->count++;
        }

        /* Check Pass/Fail */
        if(ErrData->count>=ErrData->limit)
        {
            ErrData->flag = ERR_FAILED;
            ErrData->count = ErrData->limit;
        }
        else
        {
            ErrData->flag = ERR_PASSED;
        }
    }
}
...
void _trap_6( void )
{
   ...
   else if (MCAL_CALL_TIN_DMA_SETDST_ADDRS == tin)
 {
     ...
     __asm("mov %0,%%d4" : "=d"(Channel));
      ...
      uint32 tempAddress;
      GET_DATA_REG(d4,Channel);
      GET_ADDRESS_REG(a5,tempAddress);
      Address =(uint32*)tempAddress;
#endif /* #if (_DIABDATA_C_TRICORE_ == 1U) */
#endif
