...
**  $FILENAME   : Mcu.c $                                                    **
**                                                                           **
**  $CC VERSION : \main\dev_patch_rel330\1 $                                 **
**                                                                           **
**  $DATE       : 2017-05-13 $                                               **
**                                                                           **
...
Std_ReturnType Mcu_InitClock( Mcu_ClockType ClockSetting )
{
  ...
  #endif /* (MCU_DEV_ERROR_DETECT == STD_ON) || (MCU_SAFETY_ENABLE == STD_ON)*/
  {
    ...
    else
    {
      ...
      {
         /* --------------------------------------------------------------- */
         /*               prescaler mode configuration                      */
         /* --------------------------------------------------------------- */
         /*  Errata: PLL_TC.005(PLL initialization after cold power-up) */
         MCU_SFR_INIT_RESETSAFETYENDINIT_TIMED(MCU_SAFETY_ENDINT_TIMEOUT);
         /* Set PLL to power saving mode */
         /* Set PLLPWD(bit 16) to 0*/
         MCU_SFR_INIT_MODIFY32(SCU_PLLCON0.U,MCU_PLLCON0_PLLPWD_CLEARMASK,\
                                                   MCU_PLLCON0_PLLPWD_SETMASK)
         MCU_SFR_INIT_SETSAFETYENDINIT_TIMED();
         /* delay count value is configured using parameter McuPllInitDelay. 
         User to configure such that value of macro MCU_CONF_DELAY_PLL is 
         ...
         MCU_SFR_INIT_RESETSAFETYENDINIT_TIMED(MCU_SAFETY_ENDINT_TIMEOUT);
         /* Set PLL Power Saving Mode to normal behaviour */
         /* Set PLLPWD(bit 16) to 1*/
         MCU_SFR_INIT_MODIFY32(SCU_PLLCON0.U,MCU_PLLCON0_PLLPWD_CLEARMASK,\
                                                  MCU_PLLPWD_IS_SET_TO_NORMAL) 
         MCU_SFR_INIT_SETSAFETYENDINIT_TIMED();
         /* ----- End of Errata PLL_TC.005 workaround ----- */

         /* checking PLLBYPST flag(VCOBYST-bit 0)*/
         /* checking, already in normal mode or prescalar mode */
           ...
           MCU_PLLSTAT_VCOBYST_CLEARMASK)) == ((uint32)MCU_PLLSTAT_NORMAL_MODE))
         {
           ...
           /* The input clock from the oscillator is 
        ...
        if((Mcu_lInitCcuconLckBitCheck(                                       \
           (volatile uint32 *)(void *)&SCU_CCUCON0.U)) != (uint32)MCU_BITVAL_0)
        {
          /*Set UP(bit 30) to 0   */
      ...
      else
      {
         ...
         /* Set K1DIV with 0 to have register value as 1 */
         MCU_SFR_INIT_MODIFY32(SCU_PLLCON1.U,MCU_PLLCON1_K1DIV_CLEARMASK,\
                                                   MCU_PLLCON1_K1DIV_SETMASK)
         ...
         MCU_SFR_INIT_MODIFY32(SCU_PLLCON0.U,MCU_PLLCON0_OSCDISCDIS_CLEARMASK,\
                                                MCU_PLLCON0_OSCDISCDIS_SETMASK)
         /* PLL is configured into Prescaler Mode; VCO is bypassed*/
         /* Set VCOBYP(bit 0) to 1*/
         MCU_SFR_INIT_MODIFY32(SCU_PLLCON0.U,MCU_PLLCON0_VCOBYP_CLEARMASK,\
        ...
        /* Set K2DIV(bits 0 to 6) with configured value*/
         ...
         /* PLL lock check is not done here, as per autosar requirement MCU138.
            MCU138: The function Mcu_InitClock shall start the PLL lock 
            procedure(if PLL shall be initialized) and shall return without 
            waiting until the PLL is locked.
            API Mcu_GetPllStatus() provides the status of PLL lock as per 
            autosar requirement MCU008. User of the MCU driver is expected
            to check the PLL lock status by calling Mcu_GetPllStatus() API,
            before calling Mcu_DistributePllClock() */
         ...
         /* Call platform specific initialization */
         RetValue = Mcu_lPlatformInit();
         /* switch from Fback to Fpll (fk2) done inside 
