...
IFX_LOCAL_INLINE uint16 Spi_lModuleConfigured(uint8 Module);
    ...
    if(CompareValue == SPI_INITCHK_COMP_VALUE)
    {
       ...
       do
       {
         if(Spi_lModuleConfiguredAndAsync((uint8)ModLoopCtr) != 0U)
         {
           ...
           ReadValue = (uint32)((uint32)(SPI_SFR_INIT_USER_MODE_READ32\
                                      ...
                                      Spi_HWUnitType  HwUnit, uint8 ApiAccessId)
{
  ...
  if (CSPin == SPI_CS_HW)
  {
    ...
    if (ApiAccessId == SPI_RUNTIME_ACCESS)
    {
      ...
      /*IFX_MISRA_RULE_17_04_STATUS="SPI_HW_MODULE" Pointer arithmetic is used
        to efficiently access the SFRs of multiple QSPI Modules.*/
      SPI_SFR_RUNTIME_MODIFY32(SPI_HW_MODULE[Module].SSOC.U,temp,temp)
    }
    else
    {
      /*IFX_MISRA_RULE_17_04_STATUS="SPI_HW_MODULE" Pointer arithmetic is used
                                      ...
                                      Spi_HWUnitType  HwUnit, uint8 ApiAccessId)
{
  ...
  if (CSPin == SPI_CS_HW)
  {
    ...
    if (ApiAccessId == SPI_RUNTIME_ACCESS)
    {
      /*IFX_MISRA_RULE_17_04_STATUS="SPI_HW_MODULE" Pointer arithmetic is used
       to efficiently access the SFRs of multiple QSPI Modules.*/
      SPI_SFR_RUNTIME_MODIFY32(SPI_HW_MODULE[Module].SSOC.U,Temp,(uint32)0U)
    }
    else
#endif /*(SPI_USER_MODE_RUNTIME_API_ENABLE == STD_ON)*/
    {
      ...
      /*IFX_MISRA_RULE_17_04_STATUS="SPI_HW_MODULE" Pointer arithmetic is used
       to efficiently access the SFRs of multiple QSPI Modules.*/
      SPI_HW_MODULE[Module].SSOC.U = (unsigned_int)\
                                          (SPI_HW_MODULE[Module].SSOC.U & Temp);
    }
  }
  else
...
void Spi_lHwDisableQspiErrIntr(uint32 Module)
{
 ...
 if(((uint32)((uint32)\
          SPI_SFR_RUNTIME_USER_MODE_READ32(MODULE_SRC.QSPI.QSPI[Module].ERR.U)\
...
IFX_LOCAL_INLINE void Spi_lHwDisableQspiRxIntr(uint32 Module, uint8 ApiAccessId)
{
  ...
  {
    #ifdef IFX_SPI_DEBUG
    RxSreStatus = (((uint32)((uint32)(SPI_SFR_RUNTIME_USER_MODE_READ32\
...
IFX_LOCAL_INLINE void Spi_lHwEnableQspiTxIntr(uint32 Module)
{
  ...
  SPI_SFR_RUNTIME_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].TX.U,\
...
IFX_LOCAL_INLINE void Spi_lHwDisableQspiPtIntr_SV(uint32 Module)
{
  if(((uint32)(((uint32)SPI_SFR_DEINIT_USER_MODE_READ32\
     (MODULE_SRC.QSPI.QSPI[Module].PT.U)) & SPI_SRC_QSPI_PT_CLEARMASK1)\
                                               >>SPI_SRC_QSPI_PT_BITPOS)  == 1U)
  {
    /*Reserve bit access is ensured for SRC_QSPImPT register*/
    SPI_SFR_DEINIT_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].PT.U,\
                                                              ...
                                                              uint8 ApiAccessId)
{
  ...
  if (ApiAccessId == SPI_RUNTIME_ACCESS)
  {
    ...
    /*Reserve bit access is ensured for SRC_QSPImU register*/
      SPI_SFR_RUNTIME_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].U.U,\
...
{
    /*Reserve bit access is ensured for SRC_QSPImU register*/
  SPI_SFR_DEINIT_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].U.U,\
...
IFX_LOCAL_INLINE void Spi_lHwInitIntr_SV(uint32 Module)
{
  ...
  /*Reserve bit access is ensured for SRC_QSPImTX register*/
   ...
   SPI_SFR_INIT_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].TX.U,\
                            SPI_SRC_QSPI_RX_CLEARMASK4,SPI_SRC_QSPI_RX_SETMASK4)
...
{
  /*Reserve bit access is ensured for SRC_QSPImRX register*/
  SPI_SFR_DEINIT_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].RX.U,\
                           SPI_SRC_QSPI_RX_CLEARMASK7,SPI_SRC_QSPI_RX_SETMASK9)
  SPI_SFR_DEINIT_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].RX.U,\
                                                 SPI_SRC_QSPI_RX_CLEARMASK6,0U)
  /*Reserve bit access is ensured for SRC_QSPImTX register*/
  SPI_SFR_DEINIT_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].TX.U,\
                           SPI_SRC_QSPI_RX_CLEARMASK7,SPI_SRC_QSPI_RX_SETMASK9)
  SPI_SFR_DEINIT_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].TX.U,\
                                                 SPI_SRC_QSPI_RX_CLEARMASK6,0U)
   /*Reserve bit access is ensured for SRC_QSPImERR register*/
  SPI_SFR_DEINIT_MODIFY32(MODULE_SRC.QSPI.QSPI[Module].ERR.U,\
                                                 SPI_SRC_QSPI_RX_CLEARMASK6,0U)
}
#endif
/*(SPI_LEVEL_DELIVERED == 1U) || (SPI_LEVEL_DELIVERED == 2U)*/
...
static void Spi_lHwDmaResetSfr(void)
{
  ...
  do
  {
    if(Spi_lModuleConfiguredAndAsync((uint8)ModLoopCtr) != 0U)
    {
      ...
      SPI_SFR_INIT_WRITE32(MODULE_DMA.CH[DmaTransmitChannel].\
                                                  ADICR.U,SPI_DMA_RESET_VALUE);
      SPI_SFR_INIT_WRITE32(MODULE_DMA.CH[DmaTransmitChannel].\
                                                  CHCSR.U,SPI_DMA_RESET_VALUE);
      /*Reserve bit access is ensured for DMA_TSRz register*/
      SPI_SFR_INIT_WRITE32(MODULE_DMA.TSR[DmaTransmitChannel].U,\
                                                        SPI_DMA_RESET_VALUE);
      SPI_SFR_INIT_WRITE32(MODULE_DMA.CH[DmaTransmitChannel].\
      ...
      SPI_SFR_INIT_WRITE32(MODULE_DMA.CH[DmaReceiveChannel].\
                                                ADICR.U,SPI_DMA_RESET_VALUE);
      SPI_SFR_INIT_WRITE32(MODULE_DMA.CH[DmaReceiveChannel].\
                                                CHCSR.U,SPI_DMA_RESET_VALUE);
      /*Reserve bit access is ensured for DMA_TSRz register*/
      SPI_SFR_INIT_WRITE32(MODULE_DMA.TSR[DmaReceiveChannel].\
                                                        U,SPI_DMA_RESET_VALUE);
      SPI_SFR_INIT_WRITE32(MODULE_DMA.CH[DmaReceiveChannel].SADR.\
                                                        U,SPI_DMA_RESET_VALUE);
      SPI_SFR_INIT_WRITE32(MODULE_DMA.CH[DmaReceiveChannel].CHCFGR\
                                                        .U,SPI_DMA_RESET_VALUE);
      SPI_SFR_INIT_WRITE32(MODULE_DMA.CH[DmaReceiveChannel].\
                                                    DADR.U,SPI_DMA_RESET_VALUE);
    }
...
IFX_LOCAL_INLINE void Spi_lHwInitDmaIntr(uint32 Module)
{
  ...
  SPI_SFR_RUNTIME_MODIFY32(*DmaChAddr,SPI_DMACHADDR_CLEARMASK,\
  ...
  DmaChAddr = (volatile uint32 *)(volatile void *)(SRC_DMACH0ADDR +           \
    ...
    /* Set SRC.IOVCLR, SRC.SWSCLR, SRC.CLRR */
    /* Clear SRC.SRE */
   SPI_SFR_RUNTIME_MODIFY32(*DmaChAddr,SPI_DMACHADDR_CLEARMASK1,\
                                                          SPI_DMACHADDR_SETMASK)
}
...
IFX_LOCAL_INLINE void Spi_lHwInitDmaIntr_SV(uint32 Module)
{
  ...
  /*IFX_MISRA_RULE_17_04_STATUS="MoveEngCh0BaseAddr" Pointer arithmetic is used
    to efficiently access the SFRs of multiple QSPI Modules.*/
  IntrSrc = SPI_SFR_INIT_USER_MODE_READ32(*((volatile uint32*)\
  ...
  /*IFX_MISRA_RULE_17_04_STATUS="MoveEngCh0BaseAddr" Pointer arithmetic is used
    to efficiently access the SFRs of multiple QSPI Modules.*/
  SPI_SFR_INIT_WRITE32(*((volatile uint32 *)(volatile void *)\
...
IFX_LOCAL_INLINE void Spi_lHwDeInitDmaIntr_SV(uint32 Module)
{
  ...
  /*IFX_MISRA_RULE_17_04_STATUS="MoveEngCh0BaseAddr" Pointer arithmetic is used
    to efficiently access the SFRs of multiple QSPI Modules.*/
  IntrSrc = SPI_SFR_DEINIT_USER_MODE_READ32(*((volatile uint32*)\
                        (volatile void*)MoveEngCh0BaseAddr + RxChannelOffset));
  ...
  /*IFX_MISRA_RULE_17_04_STATUS="MoveEngCh0BaseAddr" Pointer arithmetic is used
    to efficiently access the SFRs of multiple QSPI Modules.*/
  SPI_SFR_DEINIT_WRITE32(*((volatile uint32*)\
  ...
  /*IFX_MISRA_RULE_17_04_STATUS="MoveEngCh0BaseAddr" Pointer arithmetic is used
    to efficiently access the SFRs of multiple QSPI Modules.*/
  SPI_SFR_DEINIT_WRITE32(*((volatile uint32 *)(volatile void *)\
                                 MoveEngCh0BaseAddr + TxChannelOffset),IntrSrc);
}
...
IFX_LOCAL_INLINE void Spi_lHwDmaClearIntr(Dma_ChannelType Channel)
{
  ...
  /*IFX_MISRA_RULE_17_04_STATUS="MoveEngCh0BaseAddr" Pointer arithmetic is used
    to efficiently access the SFRs of multiple QSPI Modules.*/
  IntrSrc = SPI_SFR_RUNTIME_USER_MODE_READ32(*((volatile uint32*)\
  ...
  /*IFX_MISRA_RULE_17_04_STATUS="MoveEngCh0BaseAddr" Pointer arithmetic is used
    ...
    SPI_SFR_RUNTIME_MODIFY32(*RegAddress,SPI_REGADDRESS_CLEARMASK,\
                                              ...
                                              uint32 Address, uint8 ApiAccessId)
{
  ...
  {
    SPI_SFR_INIT_WRITE32(MODULE_DMA.CH[Channel].SADR.U,\
                                                        (uint32)RemapAddress);
  }
  else
  {
    SPI_SFR_DEINIT_WRITE32(MODULE_DMA.CH[Channel].SADR.U,\
                                                        (uint32)RemapAddress);
  }
}
...
IFX_LOCAL_INLINE void Spi_lDmaChClrCtlCfgInit(Dma_ChannelType Channel)
{
  ...
  ltemp = (uint32)SPI_SFR_INIT_USER_MODE_READ32(MODULE_DMA.TSR[Channel].U);
...
IFX_LOCAL_INLINE void Spi_lDmaChClrCtlCfgDeinit(Dma_ChannelType Channel)
{
  ...
  ltemp = (uint32)SPI_SFR_DEINIT_USER_MODE_READ32(MODULE_DMA.TSR[Channel].U);
  /* Clear ECH bit, Set DCH bit */
  ltemp &= (uint32)(~((uint32)1U << IFX_DMA_TSR_ECH_OFF));
  ltemp |= ((uint32)1U << IFX_DMA_TSR_DCH_OFF);
  /* Reset */
                                              ...
                                              uint32 Address, uint8 ApiAccessId)
{
  ...
  if (ApiAccessId == SPI_INIT_ACCESS)
  {
    SPI_SFR_INIT_WRITE32(MODULE_DMA.CH[Channel].DADR.U,\
                                                        (uint32)RemapAddress);
  }
  else
  {
    SPI_SFR_DEINIT_WRITE32(MODULE_DMA.CH[Channel].DADR.U,\
                                                        (uint32)RemapAddress);
  }
}
...
IFX_LOCAL_INLINE void Spi_lDmaChClrIntrFlagsInit(Dma_ChannelType Channel)
{
  ...
  /*Reserve bit access is ensured for DMA_TSRz register*/
  SPI_SFR_INIT_MODIFY32(MODULE_DMA.TSR[Channel].U,\
...
IFX_LOCAL_INLINE void Spi_lDmaCfgMeErrIntr(uint8 Enable)
{
  ...
  */
  SPI_SFR_INIT_MODIFY32(MODULE_DMA.ERRINTR.U,\
                                                                    ...
                                                                    ApiAccessId)
{
  ...
  */
  ...
  if (ApiAccessId == SPI_RUNTIME_ACCESS)
  {
    DmaMovEnErrBits  = (uint32)((uint32)(SPI_SFR_RUNTIME_USER_MODE_READ32\
                                                                    ...
                                                                    ApiAccessId)
{
  ...
  if (ApiAccessId == SPI_RUNTIME_ACCESS)
  {
    SPI_SFR_RUNTIME_USER_MODE_WRITE32(GetPortAddressPtr->OMR.U,Temp);
  }
  else
  #endif /*(SPI_USER_MODE_RUNTIME_API_ENABLE == STD_ON)*/
  ...
  if (ApiAccessId == SPI_DEINIT_ACCESS)
  {
    SPI_SFR_DEINIT_USER_MODE_WRITE32(GetPortAddressPtr->OMR.U,Temp);
  }
  else
...
static Std_ReturnType Spi_lHwDmaSfrInitCheck(void)
{
  ...
  do
  {
    if(Spi_lModuleConfiguredAndAsync((uint8)ModLoopCtr) != 0U)
    {
      ...
      RemapAddress = (uint32)Mcal_GetDsprReMapAddress(Address);
      ReadValue = (uint32)(SPI_SFR_INIT_USER_MODE_READ32(MODULE_DMA.CH\
                                                   [DmaReceiveChannel].SADR.U));
      CompareValue &= (ReadValue ^ ~(RemapAddress));

      /* Check channel control register */
      ReadValue = (uint32)(SPI_SFR_INIT_USER_MODE_READ32(MODULE_DMA.\
