...
void Gpt_Init(const Gpt_ConfigType *ConfigPtr)
{
  ...
  /*(GPT_DEV_ERROR_DETECT == STD_ON) || ( GPT_SAFETY_ENABLE == STD_ON )*/
  {
    ...
    */
    ...
    {
      /* The AssignedHwUnit contains TOM channel used */
...
static void Gpt_lGtmStopTimer(Gpt_ChannelType Channel)
{
  ...
  /* Update TOMi_TGCx_ENDIS_STAT register*/
  /*IFX_MISRA_RULE_11_05_STATUS=volatile in terms of pointer access.
  Permitted for special function registers.*/
  GPT_SFR_RUNTIME_USER_MODE_WRITE32((((*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).
                                          ...
                                          )
{
  ...
  if((TimeOutVal <= GPT_GTM_TOM_TIMER_MAX) && (TimeOutVal > 0U))
  {
    ...
    /* Get address of TOMi_TGCx_ENDIS_STAT register */
  ...
  if (Gpt_kConfigPtr == ConfigPtr)
  {
    ...
    for(Channel = 0U; Channel < Gpt_kConfigPtr->GptMaxChannels; Channel++)
    {
        ...
        if (Gpt_lModeOsm(ChannelMode))
        {
          ...
          CompareFlag &= ((~TempFlag) ^ (uint32)GPT_SFR_INIT_USER_MODE_READ32\
                                         ((TomChannelRegPtr->IRQ_NOTIFY.U)));
          TempFlag = 1UL;
