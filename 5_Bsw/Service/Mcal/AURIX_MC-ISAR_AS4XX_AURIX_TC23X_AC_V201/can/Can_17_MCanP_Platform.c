...
**                                                                           **
**  $CC VERSION : \main\dev_tc29xbc_as4.0.3\22 $                             **
**                                                                           **
**  $DATE       : 2017-04-05 $                                               **
**                                                                           **
                 ...
                 uint16 CanControllerFDBaudrate,uint16 CanControllerTxDelayComp)
{
    ...
    #if(CAN_HARDWARE_ISO_SUPPORT == STD_OFF)
    UNUSED_PARAMETER(KerIndex)
    #endif
}
...
** Reentrancy       : Non-Reentrant                                           **
**                                                                            **
** Parameters (in)  : KerIdx - CAN Kernel Id                                  **
...
uint8 Can_lGetHthIndex(uint8 KerIdx,uint8 HthIdx)
{
  ...
  #endif /*#if (CAN_TX_OBJECT_KER0 > 0U)*/
  #if (CAN_TX_OBJECT_KER1 > CAN_ZERO)
  if(CAN_ONE == KerIdx)
  {
    /* Calculate the HTH Id mapped to corresponding HW Object ID */
    /*IFX_MISRA_RULE_17_04_STATUS=Pointer arithmetic used due to
     PBConfigStructure and is within allowed range.*/
    TxMOIdx = Can_kConfigPtr->CanTxObjectHthKer1MapPtr[HthIdx];
  }
  #endif /*#if (CAN_TX_OBJECT_KER1 > 0U) */
  #if((CAN_TX_OBJECT_KER0 < CAN_ONE) && (CAN_TX_OBJECT_KER1 < CAN_ONE))
...
** TRACEABILITY: [cover parentID=]                                            **
**                                                                            **
** Syntax           : IFX_INLINE uint8 Can_lGetHrhIndex                       **
**                                               (                            **
**                                                 uint8 KerIdx,              **
**                                                 uint8 HrhOffset            **
**                                               )                            **
...
uint8 Can_lGetHrhIndex(uint8 KerIdx,uint8 HrhOffset)
{
  ...
  #endif
  #if (CAN_RX_OBJECT_KER1 > CAN_ZERO)
  if(CAN_ONE == KerIdx)
  {
    /* Calculate the HTH Id mapped to corresponding HW Object ID */
    /*IFX_MISRA_RULE_17_04_STATUS=Pointer arithmetic used due to
    PBConfigStructure and is within allowed range.*/
    Hrh = Can_kConfigPtr->CanRxObjectHrhKer1MapPtr[HrhOffset];
  }
  #endif
  #if((CAN_RX_OBJECT_KER0 < CAN_ONE) && (CAN_RX_OBJECT_KER1 < CAN_ONE))
